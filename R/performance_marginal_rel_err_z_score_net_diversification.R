...
for (parameter in 1:24) { # 1:12 - relative errors; 13:24 - z-scores
  ...
  err_plots[[length(err_plots) + 1]] <- ggplot(df, aes(x = x, y = y)) +
    ...
    geom_ribbon(
      ...
      aes(x = x, ymin = ymin, ymax = ymax),
      fill = c("#B1CAE1", "#F28E2B", "#B1CAE1", "#F28E2B")[ceiling(parameter / 6)],
      alpha = 0.6, #0.4,
      inherit.aes = FALSE
    ) +
    geom_line(
      ...
      linewidth = 0.7,
      color = c("#4E79A7", "#F28E2B", "#4E79A7", "#F28E2B")[ceiling(parameter / 6)]
    ) +
    geom_line(
    ...
    geom_text(
      ...
      aes(y = y, label = label),
      hjust = -0.1, size = 2, color = c("#4E79A7", "#F28E2B", "#4E79A7", "#F28E2B")[ceiling(parameter / 6)],
      fontface = "bold"
    ) +
    annotate(
      "text",
