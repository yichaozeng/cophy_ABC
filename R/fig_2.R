...
# here we generate the extinction fractions randomly
...
for (i in 1:2) {
  ...
  cophys <- c(
    cophys,
    list(sim_cophyBD(hbr = lambda_H, hdr = mu_H, sbr = lambda_S, sdr = mu_S, host_exp_rate = lambda_W, cosp_rate = lambda_C, time_to_sim = 2, numbsim = 1, hs_mode = T))
  )
...
cophys <- list.load(file = 'cophy.rds')
...
samples_1_blend_1 <- SS_norm(cophys[[1]], breaks = breaks, tr_ht = tr_ht)[[1]][1:n_bin]
samples_2_blend_1 <- SS_norm(cophys[[2]], breaks = breaks, tr_ht = tr_ht)[[1]][1:n_bin]

samples_1_blend_2 <- SS_norm(cophys[[1]], breaks = breaks, tr_ht = tr_ht)[[2]][1:n_bin]
samples_2_blend_2 <- SS_norm(cophys[[2]], breaks = breaks, tr_ht = tr_ht)[[2]][1:n_bin]

# plotting
x <- (breaks[-1] + breaks[-length(breaks)]) / (2 * tr_ht)

y1_blend1 <- samples_1_blend_1 * 2 / n_bin
y2_blend1 <- samples_2_blend_1 * 2 / n_bin

y1_blend2 <- samples_1_blend_2 * 2 / n_bin
y2_blend2 <- samples_2_blend_2 * 2 / n_bin

# here there is a loop for plottting both BLenD_a and BLenD_b
blend_plots <- NULL
...
for(part in c('a','b')){
  ...
  if(part == 'a'){
    ...
    y2 <- y2_blend1
    title <- "BLenD curve a"
    x_label_text <- expression(paste(d[a], sep = ''))
  }else if(part == 'b'){
    y1 <- y1_blend2
    y2 <- y2_blend2
    title <- "BLenD curve b"
    x_label_text <- expression(paste(delta[b], sep = ''))
  }
  
  upper <- y1 * (y1 > y2) + y2 * (y1<=y2)
  lower <- y2 * (y1 > y2) + y1 * (y1<=y2)
  
  # print the area of shade
  print(sum(upper - lower))
  ...
  # save the plot to the list of plots
  blend_plots[[length(blend_plots) + 1]] <- plot_curves
  
}

print(paras)

# arrange the two panels and print the plot
temp <- ggarrange(blend_plots[[1]], blend_plots[[2]],
          ncol = 1, nrow = 2,
          #labels = c("a", "b"),
          common.legend = TRUE, legend = "none") # share legend
...
pdf("fig2_curves.pdf", width = 4, height = 7, pointsize = 12)
print(temp)
dev.off()

# save the trees
list.save(cophys, file = 'cophy.rds')
